...
Locale-aware: `reason` is written in the client's configured analysis_language.
classification, confidence, was_abrupt, and abandonment_trigger remain canonical codes.
"""
...
def _build_prompt(language: str) -> str:
    ...
    lang_note = (
        f"LANGUAGE NOTE: Write the `reason` field in {language}. "
        f"Keep classification, confidence, and abandonment_trigger as the exact English codes listed above.\n\n"
    )
...
async def analyze(
    ...
    *,
    ...
    Args:
        ...
        language: Output language for the `reason` field.
            classification, confidence, and abandonment_trigger stay canonical.
    """
