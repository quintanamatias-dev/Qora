...
def test_update_filler_tracks_last_filler():
    ...
    store.update_filler("conv-001", "A ver...")

    state = store.get("conv-001")
...
def test_filler_dedup_no_consecutive_repeat():
    ...
    filler1 = select_filler(store.get("conv-001"))
    store.update_filler("conv-001", filler1)

    # Get second filler — must be different
    filler2 = select_filler(store.get("conv-001"))
    assert filler2 != filler1, (
