/**
 * AgentsSection — Route-scoped agents management section
 *
 * Replaces the old AgentsPanel which had a client selector dropdown.
...
export function AgentsSection({ clientId }: AgentsSectionProps) {
  const { data: agents, isLoading: agentsLoading, isError: agentsError } = useAgents(clientId)
    ...
    <div className="space-y-6 pt-2">
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          ...
          <code className="font-mono text-xs text-teal">{clientId}</code>
        </div>
            ...
            <TableBody>
              {agents.map((agent) => (
                <TableRow key={agent.agent_id}>
                  ...
                  <TableCell>
                    <div
                      className="text-xs text-ink-3 space-y-0.5"
                      data-testid={`voice-tuning-${agent.agent_id}`}
                    >
                      <div>
                        Speed: <span className="font-medium text-ink">{agent.tts_speed}</span>
                      </div>
                      <div>
                        Stability:{' '}
                        <span className="font-medium text-ink">{agent.tts_stability}</span>
                      </div>
                      <div>
                        Similarity:{' '}
                        <span className="font-medium text-ink">{agent.tts_similarity_boost}</span>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <Input
                ...
                onChange={(e) =>
                  setCreateForm((f) => ({
                    ...f,
                    tts_stability: parseFloat(e.target.value) || 0.4,
                  }))
                }
              />
              <Input
